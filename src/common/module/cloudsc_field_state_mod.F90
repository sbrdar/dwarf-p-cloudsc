...
MODULE CLOUDSC_FIELD_STATE_MOD
  ...
  USE FIELD_MODULE, ONLY: FIELD_2D, FIELD_3D, FIELD_4D, FIELD_INT2D, FIELD_LOG2D, malloc_host_pinned_4d
  ...
  TYPE CLOUDSC_FIELD_STATE
    ...
    REAL(KIND=JPRB), POINTER, CONTIGUOUS :: DATA_RDONLY(:,:,:,:)
    REAL(KIND=JPRB), POINTER, CONTIGUOUS :: DATA_RWONLY(:,:,:,:)

    ! Storage fields to provide thread-local views
    CLASS(FIELD_2D), POINTER :: F_PRAINFRAC_TOPRFZ, F_PLSM
    CLASS(FIELD_INT2D), POINTER :: F_KTYPE
    CLASS(FIELD_LOG2D), POINTER :: F_LDCUM
    CLASS(FIELD_3D), POINTER :: F_PLCRIT_AER, F_PICRIT_AER, F_PRE_ICE, F_PCCN, F_PNICE, F_PT, F_PQ, &
    ...
    CLASS(FIELD_3D), POINTER :: F_PFSQLF, F_PFSQIF, F_PFCQLNG, F_PFCQNNG, F_PFSQRF, F_PFSQSF, F_PFCQRNG, &
     & F_PFCQSNG, F_PFSQLTUR, F_PFSQITUR, F_PFPLSL, F_PFPLSN, F_PFHPSL, F_PFHPSN
    CLASS(FIELD_4D), POINTER :: F_PCLV, F_TENDENCY_TMP, F_TENDENCY_LOC
  CONTAINS
...
CONTAINS
  ...
  FUNCTION CREATE_FIELD_ALLOCATE_INT2D(SHAPE, NBLOCKS) RESULT(FIELD_PTR)
    CLASS(FIELD_INT2D), POINTER :: FIELD_PTR
    INTEGER(KIND=JPIM), INTENT(IN) :: SHAPE, NBLOCKS
    INTEGER(KIND=JPIM) :: B
  ...
  FUNCTION CREATE_FIELD_ALLOCATE_2D(SHAPE, NBLOCKS, ZERO) RESULT(FIELD_PTR)
    CLASS(FIELD_2D), POINTER :: FIELD_PTR
    INTEGER(KIND=JPIM), INTENT(IN) :: SHAPE, NBLOCKS
  ...
  FUNCTION CREATE_FIELD_ALLOCATE_3D(SHAPE, NBLOCKS, ZERO) RESULT(FIELD_PTR)
    CLASS(FIELD_3D), POINTER :: FIELD_PTR
  ...
  FUNCTION CREATE_FIELD_ALLOCATE_4D(SHAPE, NBLOCKS, ZERO) RESULT(FIELD_PTR)
    CLASS(FIELD_4D), POINTER :: FIELD_PTR
  ...
  FUNCTION CREATE_FIELD_WRAP_PACKED_3D(DATA, IDX) RESULT(FIELD_PTR)
    ...
    CALL FIELD_PTR%INIT(DATA=DATA, IDX=IDX)
...
!$omp end parallel do 
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG, USE_PACKED)
    ...
    IF (LLPACKED) THEN
      ...
      SELF%DATA_RDONLY => MALLOC_HOST_PINNED_4D([NPROMA, SELF%KLEV, NFIELDS], SELF%NBLOCKS)
      ...
      SELF%DATA_RWONLY => MALLOC_HOST_PINNED_4D([NPROMA, SELF%KLEV+1, NFIELDS], SELF%NBLOCKS)
      SELF%DATA_RWONLY(:,:,:,:) = 0.0_JPRB
    ...
    SELF%TENDENCY_LOC%F_T => CREATE_FIELD_ALLOCATE_3D(SHAPE=[NPROMA,SELF%KLEV], NBLOCKS=SELF%NBLOCKS, ZERO=.TRUE.)
    SELF%TENDENCY_LOC%F_A => CREATE_FIELD_ALLOCATE_3D(SHAPE=[NPROMA,SELF%KLEV], NBLOCKS=SELF%NBLOCKS, ZERO=.TRUE.)
    SELF%TENDENCY_LOC%F_Q => CREATE_FIELD_ALLOCATE_3D(SHAPE=[NPROMA,SELF%KLEV], NBLOCKS=SELF%NBLOCKS, ZERO=.TRUE.)
    SELF%TENDENCY_LOC%F_CLD => CREATE_FIELD_ALLOCATE_4D(SHAPE=[NPROMA,SELF%KLEV,NCLV], NBLOCKS=SELF%NBLOCKS, ZERO=.TRUE.)

    SELF%TENDENCY_TMP%F_T => CREATE_FIELD_ALLOCATE_3D(SHAPE=[NPROMA,SELF%KLEV], NBLOCKS=SELF%NBLOCKS)
    SELF%TENDENCY_TMP%F_A => CREATE_FIELD_ALLOCATE_3D(SHAPE=[NPROMA,SELF%KLEV], NBLOCKS=SELF%NBLOCKS)
    SELF%TENDENCY_TMP%F_Q => CREATE_FIELD_ALLOCATE_3D(SHAPE=[NPROMA,SELF%KLEV], NBLOCKS=SELF%NBLOCKS)
    SELF%TENDENCY_TMP%F_CLD => CREATE_FIELD_ALLOCATE_4D(SHAPE=[NPROMA,SELF%KLEV,NCLV], NBLOCKS=SELF%NBLOCKS)
